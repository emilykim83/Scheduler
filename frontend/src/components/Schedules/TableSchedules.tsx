...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/schedules/schedulesSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureSchedulesCols';
...
const TableSampleSchedules = ({
  ...
  useEffect(() => {
    if (schedulesNotify.showNotification) {
      notify(
        schedulesNotify.typeNotification,
        schedulesNotify.textNotification,
      );
    }
  }, [schedulesNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/schedules/schedules-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/schedules/schedules-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
    ...
    <>
      ...
      {!showGrid && (
        <BigCalendar
          ...
          pathEdit={`/schedules/schedules-edit/?id=`}
          pathView={`/schedules/schedules-view/?id=`}
...
export default TableSampleSchedules;
