...
const EditSchedules = () => {
  ...
  const { schedulesId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: schedulesId }));
  }, [schedulesId]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: schedulesId, data }));
...
EditSchedules.getLayout = function getLayout(page: ReactElement) {
  return (
